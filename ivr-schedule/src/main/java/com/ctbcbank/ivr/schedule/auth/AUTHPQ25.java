...
public class AUTHPQ25 extends Auth {
	...
	public AUTHPQ25(NamedParameterJdbcTemplate namedParameterJdbcTemplate, Logger logger,
...
//			銀行名稱
			...
			while(batchArgsForInsert.size()>=5000) {
				namedParameterJdbcTemplate.batchUpdate(authProperties.getAuthpq25InsertSql(), batchArgsForInsert.toArray(new MapSqlParameterSource[0]));
		...
		logger.info("Authpq25 insert times : {}", insertCount);
		params.put("FilePath", getFileName());
		params.put("SuccessCount", insertCount);
		params.put("FailCount", "0");
		namedParameterJdbcTemplate.update(authProperties.getAuthpq25BatchSql(), params);
	}
}
